...
public class LRTAStarAlgorithm {
    ...
    public LRTAStarAlgorithm(LocationTile start, LocationTile goal, ArrayList<LocationTile> obstacleLocationList) {
        ...
        while (true) {
            if (algorithm(currentTile)) {
                LrtaGui.displayMessge("Path found!");
                break;
            }
        }
    }
    ...
    boolean algorithm(LocationTile s_dash) {
        ...
        // GUI work
        LrtaGui.tiles.get(nextTile.getIndex()).setBackground(Color.CYAN);
        LrtaGui.displayMessge("At tile " + currentTile.getTileLocation()[0] + ", " + currentTile.getTileLocation()[1]);
  ...
  /*  void getTotalCost(LocationTile current) {

        Action[] validAction = getValidActions(current);
        int[] location;
        int[] newLocation;

        location = current.getTileLocation();

        for (Action a : validAction) {
            if (a == null)
                continue;
            switch (a) {
                case TOP:
                    ...
                    result[0] = result[0] + calculateHeuristic(newLocation);
                ...
                case TOP_RIGHT:
                    ...
                    result[1] = result[1] + calculateHeuristic(newLocation);
                ...
                case RIGHT:
                    ...
                    result[2] = result[2] + calculateHeuristic(newLocation);
                ...
                case BOTTOM_RIGHT:
                    ...
                    result[3] = result[3] + calculateHeuristic(newLocation);
                ...
                case BOTTOM:
                    ...
                    result[4] = result[4] + calculateHeuristic(newLocation);
                ...
                case BOTTOM_LEFT:
                    ...
                    newLocation[0] = location[0] + 1;
                    newLocation[1] = location[1] - 1;
                    result[5] = result[5] + calculateHeuristic(newLocation);
                ...
                case LEFT:
                    ...
                    result[6] = result[6] + calculateHeuristic(newLocation);
                    break;
                case TOP_LEFT:
                    newLocation = new int[2];
                    newLocation[0] = location[0] - 1;
                    ...
                    result[7] = result[7] + calculateHeuristic(newLocation);
   ...
   /*
    public boolean lrta_agent(LocationTile s_dash) {
        if (s_dash.getTileLocation().equals(goal.getTileLocation())) {
            return true;
        }
        if (H.size() == 0) {
            H.put(s_dash, calculateHeuristic(s_dash));
        }
        if (s != null) {
            .put(s, a);
            H.put(s, getMinLRTACost);
        }
        *//*
        * Update a
        * *//*
    ...
    public int getLRTACost(LocationTile s, Action a, LocationTile s_dash, result, H) {
        if (s_dash == null) {
            return H.get(s);
        } else {
            return c(s, a, s_dash) + H[s_dash];
        }
        return cost;
    }
